...
export default {
  ...
  methods:{
   ...
   removeRoleFn(r) {
    ...
    async updateColabs(){
      const updateColabs = await this.$axios.get("getcolabs", {projectID: 46});
      ...
      if(!resUser.error){
          if(resUser.data.length>0){
            for(var i = 0; i>=resUser.data.length;i++){
              this.colabs.push({name: res.data[i].name, role: resUser.data[i].roleName, weight: resUser.data[i].roleWeight});
            }
          }
